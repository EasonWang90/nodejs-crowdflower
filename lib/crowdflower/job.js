...
Job.prototype.copyjob = function(jobid,allUnit,gold,callback) {
    ...
    if(arguments.length === 2){
        callback = allUnit;
        var path = '/v1/jobs/' + jobID + '/copy?&key=' + this.apiKey;
    }
    else{
        allUnit = allUnit.toString();
        if(allUnit == 'true'){
            var path = '/v1/jobs/' + jobID + '/copy?all_units=' + allUnit + '&key=' + this.apiKey;
        }
        else{
            var path = '/v1/jobs/' + jobID + '/copy?gold=' + gold + '&key=' + this.apiKey;
        }
    }
    //console.log(path);
